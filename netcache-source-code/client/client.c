...
uint32_t write_skew = 0; // 0 is uniform write, 1 is skewed write
...
// dynamic workload
uint64_t random_workload_mapping[CACHE_SIZE];
...
struct hot_key_message hot_key_msgs[CHANGE_SIZE];
...
// generate dynamic load
static void *change_load (void* empty) {
    uint64_t i, key;
    while (1) {
        // sleep until next change
        sleep(CHANGE_INTERVAL-1);

        // change pattern
        if (CHANGE_PATTERN == 0) {
            for (i = 0; i < CHANGE_SIZE; i++) {
                hot_key_msgs[i].key_new = - i - circular_position - 1;
                hot_key_msgs[i].key_old = - i + CACHE_SIZE
                    - circular_position - 1;
            }
            circular_position += CHANGE_SIZE;
        } else if (CHANGE_PATTERN == 1) {
            for (i = 0; i < CHANGE_SIZE; i++) {
                hot_key_msgs[i].key_new = i + CACHE_SIZE + circular_position;
                hot_key_msgs[i].key_old = i + circular_position;
            }
            circular_position += CHANGE_SIZE;
        } else {
            for (i = 0; i < CHANGE_SIZE; i++) {
                uint64_t index = rte_rand() % CACHE_SIZE;
                uint64_t key = rte_rand() % KEY_SPACE_SIZE;
                hot_key_msgs[i].key_new = key;
                hot_key_msgs[i].key_old = random_workload_mapping[index];
                random_workload_mapping[index] = key;
            }
        }
        ...
        sleep(1);
        for (i = 0; i < CHANGE_SIZE; i++) {
            hot_key_msgs[i].key_new = StringToHash(&hot_key_msgs[i].key_new, 8)
                % KEY_SPACE_SIZE;
            hot_key_msgs[i].key_old = StringToHash(&hot_key_msgs[i].key_old, 8)
                % KEY_SPACE_SIZE;
            send(controller_socket, &hot_key_msgs[i],
                sizeof(hot_key_msgs[i]), 0);
        }

        printf("%lld change pattern %d shift %"PRIu64"\n",
            (long long)time(NULL), CHANGE_PATTERN, circular_position);
    }
    pthread_exit(NULL);
}
...
static void generate_request_pkt_trace(uint32_t lcore_id, struct rte_mbuf *mbuf,
    uint32_t cur_kv_trace_p, uint64_t* kv_trace) {
    ...
    if (zipf_alpha == 0
        ...
        || (message_header->type == TYPE_PUT_REQUEST && write_skew == 0)) {
            key = (rte_rand() % PER_NODE_KEY_SPACE_SIZE) * NODE_NUM + backend_id;
    } else {
    ...
    // only-cache mode
    /*
    uint64_t key = kv_trace[kv_trace_idx];
    kv_trace_idx++;
    if (unlikely(kv_trace_idx == kv_trace_length)) {
        kv_trace_idx = 0;
    }*/
    ...
    // change pattern mode
    uint64_t zipf_key = 0;
    if (zipf_alpha == 0 || rte_rand() % 100 > kv_trace_p) {
            zipf_key = rte_rand() % KEY_SPACE_SIZE;
    } else {
        zipf_key = kv_trace[kv_trace_idx];
        kv_trace_idx++;
        if (unlikely(kv_trace_idx == kv_trace_length)) {
            kv_trace_idx = 0;
        }
    ...
    // change pattern: hot-in
    zipf_key -= circular_position;
    uint64_t key = StringToHash(&zipf_key, 8) % KEY_SPACE_SIZE;

    // change pattern: hot-out
    //zipf_key += circular_position;
    //uint64_t key = StringToHash(&zipf_key, 8) % KEY_SPACE_SIZE;

    // change pattern: random
    /*if(zipf_key < CACHE_SIZE) {
        zipf_key = random_workload_mapping[zipf_key];
    }
    uint64_t key = StringToHash(&zipf_key, 8) % KEY_SPACE_SIZE;*/

    message_header->key = rte_cpu_to_be_64(key);
...
static void generate_request_pkt_simple(uint32_t lcore_id,
...
/*static void generate_request_pkt_zipf(uint32_t lcore_id, struct rte_mbuf
*mbuf,
    uint32_t write_ratio, uint32_t zipf_alpha, uint32_t write_skew,
    ...
    if(zipf_alpha == 0
        ...
        key = zipf_next(zipf_gen) % KEY_SPACE_SIZE;
        if(change_pattern == 0) {
            key = (zipf_next(&zipf_state[vid]) - circular_position) %
            KEY_SPACE_SIZE;
        } else if (change_pattern == 1) {
            key = (zipf_next(&zipf_state[vid]) + circular_position) %
            KEY_SPACE_SIZE;
        } else { 
            key = zipf_next(&zipf_state[vid]) % KEY_SPACE_SIZE;
            if(key < cache_size) {
                key = key_map_get(key);
            }
        }
...
// duplex loop
static int32_t nc_client_duplex_loop(uint32_t lcore_id) {
    struct lcore_configuration *lconf = &lcore_conf[lcore_id];
    printf("%lld entering duplex loop on lcore %u\n",
        (long long)time(NULL),
        lcore_id);
    ...
    while (1) {
        ...
        /*if (unlikely(cur_tsc > next_adjust_tsc)) {
            if (lcore_id == rte_get_master_lcore()) {
                pkts_send_limit_ms = update_send_limit(pkts_send_limit_ms);
            }
            next_adjust_tsc += adjust_tsc;
        }*/
        ...
        if (pkts_send_ms < pkts_send_limit_ms) {
            mbuf = rte_pktmbuf_alloc(pktmbuf_pool);
            generate_request_pkt_trace(lcore_id, mbuf, 0, NULL);
            enqueue_pkt(lcore_id, mbuf);
            pkts_send_ms++;
        }
...
static int32_t nc_client_loop(__attribute__((unused)) void *arg) {
    ...
    //nc_client_duplex_loop(lcore_id);
    return 0;
}
...
static void custom_init_trace(void) {
    ...
    // initialize per-client trace
    kv_trace_length = 0;
    if (cache_only == 0) {
        for (i = 0; i < TRACE_LENGTH; i++) {
            sprintf(buffer, "%"PRIu64, kv_trace_master[i]);
            uint64_t key_hash = StringToHash(buffer, strlen(buffer)) % KEY_SPACE_SIZE;
            if (backend_id == (uint32_t) (key_hash % NODE_NUM)) {
                kv_trace_master[kv_trace_length++] = key_hash;
            }
        }
    } else {
        for (i = 0; i < 10000; i++) {
            sprintf(buffer, "%"PRIu64, (uint64_t) i);
            uint64_t key_hash = StringToHash(buffer, strlen(buffer)) % KEY_SPACE_SIZE;
            kv_trace_master[kv_trace_length++] = key_hash;
        }
    }
...
// initialization
static void custom_init_trace_zipf(void) {
    int i;
    char buffer[100];
    ...
    if (CHANGE_PATTERN == 2) {
        for (i = 0; i < CACHE_SIZE; i++) {
            random_workload_mapping[i] = i;
        }
    }

    // start change load thread
    pthread_t change_load_thread;
    if (pthread_create(&change_load_thread, NULL, change_load, NULL)) {
        perror("pthread_create");
        rte_exit(EXIT_FAILURE, "cannot create load change thread\n");
    }
...
// initialization
static void custom_init_zipf(void) {

    // init zipf state
    uint32_t i;
    zipf_state = malloc(n_lcores * sizeof(struct zipf_gen_state));
    for (i = 0; i < n_lcores; i++) {
        if (i == 0) {
            zipf_init(&zipf_state[i], KEY_SPACE_SIZE, zipf_alpha * 0.01,
                (i ^ rte_rdtsc()) % ((1UL << 48)));
        } else {
            zipf_init_copy(&zipf_state[i], &zipf_state[i-1],
                (i ^ rte_rdtsc()) % ((1UL << 48)));
        }
    }

    // init random pattern state
    //key_map_init(CACHE_SIZE, KEY_SPACE_SIZE);
...
static int nc_parse_args(int argc, char **argv) {
    ...
    printf("parsed arguments: port mask: %"PRIu32
        ...
        "\n",
        enabled_port_mask, write_ratio, zipf_alpha, write_skew,
        backend_id, kv_trace_p, cache_only, pkts_send_limit_ms);
    return 1;
}
