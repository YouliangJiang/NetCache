...
static void *change_load (void* empty) {
    printf("enter change load master\n");

    int sync_socket;
    sync_socket = socket(AF_INET, SOCK_STREAM , 0);
    if (sync_socket == -1) {
        rte_exit(EXIT_FAILURE, "cannot create socket\n");
    }
    int optval = 1;
    setsockopt(sync_socket, SOL_SOCKET, SO_REUSEADDR, 
         (const void *)&optval , sizeof(int));

    struct sockaddr_in sync_address;
    memset((char *) &sync_address, 0, sizeof(sync_address));
    sync_address.sin_addr.s_addr = inet_addr(CLIENT_MASTER_IP);
    sync_address.sin_family = AF_INET;
    sync_address.sin_port = htons(CLIENT_MASTER_PORT);

    if (bind(sync_socket, (struct sockaddr *) &sync_address, 
        sizeof(sync_address)) < 0) {
        rte_exit(EXIT_FAILURE, "cannot create socket\n");
    }

    if (listen(sync_socket, 5) < 0) {
        rte_exit(EXIT_FAILURE, "cannot create socket\n");
    }

    struct sockaddr_in slave_address;
    int slave_len = sizeof(slave_address);
    int slave_socket = accept(sync_socket, (struct sockaddr *) &slave_address,
        &slave_len);
    if (slave_socket < 0) {
        rte_exit(EXIT_FAILURE, "cannot create socket\n");
    }
    ...
    while (1) {
        ...
        // notify slave to apply change
        send(slave_socket, &change_msg, 1, 0);
...
static void custom_init_trace(void) {
    ...
    for (i = 0; i < NODE_NUM; i++) {
        sprintf(buffer, "192.168.12.%d", i / 64 + 1);
    ...
    controller.sin_addr.s_addr = inet_addr(CONTROLLER_IP);
    controller.sin_family = AF_INET;
    controller.sin_port = htons(CONTROLLER_PORT);
