...
static void *change_load (void* empty) {
    printf("enter change load slave\n");
    ...
    while (connect(sync_socket, (struct sockaddr *)&sync_address,
        sizeof(sync_address)) < 0) {
        sleep(5);
        printf("connecting to sync...\n");
    }
    printf("connected to client sync\n");

    char change_msg = 0;
    uint64_t i, key;
    while (1) {
        // wait for master to notify change
        recv(sync_socket, &change_msg, 1, 0);
...
static void custom_init_trace(void) {
    ...
    struct ipv4_hdr *ip = (struct ipv4_hdr *)((uint8_t*) header_template
        + sizeof(struct ether_hdr));
    char src_ip[] = CLIENT_SLAVE_IP;
    inet_pton(AF_INET, src_ip, &(ip->src_addr));
