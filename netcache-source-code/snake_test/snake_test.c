...
uint32_t mode = 0; // 0 is sender, 1 is receiver, 2 is duplex
uint32_t server_id = 0; // 0 is swhv03, 1 is swhv05
...
static void generate_request_pkt(uint32_t lcore_id,
    struct rte_mbuf *mbuf, uint64_t key) {
    ...
    message_header->seq = 0;//rte_rdtsc();
}
...
static int32_t nc_test_tx_loop(__attribute__((unused)) void *arg) {
    ...
    while (1) {
        ...
        if (pkts_send_ms < pkts_send_limit_ms) {
            ...
            generate_request_pkt(lcore_id, mbuf, cache_index);
...
// duplex loop for test
static int32_t nc_test_duplex_loop(__attribute__((unused)) void *arg) {
    uint32_t lcore_id = rte_lcore_id();
    if (lcore_id == 0) {
        nc_test_rx_loop(arg);
    } else {
...
static void nc_parse_args_help(void) {
    printf("simple_socket [EAL options] --\n"
        "  -m mode (0:TX 1:RX 2:Duplex)\n"
        "  -p port mask (>0)\n"
        "  -s server id (0 or 1)\n"
        "  -r pkts send limit ms (e.g., 10000)\n");
}

static int nc_parse_args(int argc, char **argv) {
    int opt, num;
    double fnum;
    while ((opt = getopt(argc, argv, "m:p:s:r:")) != -1) {
        switch (opt) {
        case 'm':
            num = atoi(optarg);
            if (num >= 0 && num <= 2) {
    ...
    printf("parsed arguments: mode: %"PRIu32
        ...
        "\n",
        mode, enabled_port_mask, server_id, pkts_send_limit_ms);
    return 1;
}
...
int main(int argc, char **argv) {
    ...
    case 1:
        ...
        break;
    case 2:
        rte_eal_mp_remote_launch(nc_test_duplex_loop, NULL, CALL_MASTER);
    default:
