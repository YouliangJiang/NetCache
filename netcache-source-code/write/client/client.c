...
uint32_t read_zipf_alpha = 99;
uint32_t write_zipf_alpha = 0;
...
uint64_t read_kv_trace[TRACE_LENGTH];
uint32_t read_kv_trace_length = TRACE_LENGTH;
uint32_t read_kv_trace_p = 64;
uint32_t read_kv_trace_idx = 0;

uint64_t write_kv_trace[TRACE_LENGTH];
uint32_t write_kv_trace_length = TRACE_LENGTH;
uint32_t write_kv_trace_p = 0;
uint32_t write_kv_trace_idx = 0;
...
static void generate_request_pkt_trace(uint32_t lcore_id, struct rte_mbuf *mbuf) {
    ...
    if(rte_rand() % 100 < write_ratio) {
        ...
        if (write_zipf_alpha == 0 || rte_rand() % 100 > write_kv_trace_p) {
            key = rte_rand() % KEY_SPACE_SIZE;
        } else {
            key = write_kv_trace[write_kv_trace_idx];
            write_kv_trace_idx++;
            if (unlikely(write_kv_trace_idx == write_kv_trace_length)) {
                write_kv_trace_idx = 0;
            }
        }
    } else {
        message_header->type = (uint8_t) TYPE_GET_REQUEST;

        if (rte_rand() % 100 > read_kv_trace_p) {
            key = rte_rand() % KEY_SPACE_SIZE;
        } else {
            key = read_kv_trace[read_kv_trace_idx];
            read_kv_trace_idx++;
            if (unlikely(read_kv_trace_idx == read_kv_trace_length)) {
                read_kv_trace_idx = 0;
            }
        }
    }
    ...
    // update dst ip
    ip->dst_addr = backend_ip[key_hash % NODE_NUM];
...
/*static void simulation() {
    ...
    while (1) {
        for (i = 0; i < 1000*10; i++) {
            uint64_t key = 0;
            if (rte_rand() % 100 > kv_trace_p) {
                    key = rte_rand() % KEY_SPACE_SIZE;
            } else {
                key = kv_trace_master[kv_trace_idx];
                kv_trace_idx++;
        ...
        for (i = 0; i < 15; i++) {
            for (j = 0; j < 10; j++) {
                ...
                printf("%"PRIu32":%"PRIu64"\t", idx, counters[idx]);
                idx++;
            }
            printf("\n");
            if (idx >= NODE_NUM) {
...
static void custom_init_trace(void) {
    ...
    // initialize trace
    if (read_zipf_alpha > 0) {
        sprintf(buffer, "trace_zipf_%"PRIu32"_key_1M_len_10M", read_zipf_alpha);
    ...
    read_kv_trace_length = TRACE_LENGTH;

    if (write_zipf_alpha > 0) {
        sprintf(buffer, "trace_zipf_%"PRIu32"_key_1M_len_10M",
           write_zipf_alpha);
        FILE *file = fopen(buffer, "r");
        if (file) {
            int key;
    ...
    write_kv_trace_length = TRACE_LENGTH;

    // initialize backend ip
    for (i = 0; i < NODE_NUM; i++) {
        sprintf(buffer, "192.168.12.1");
    ...
    printf("target rate: %"PRIu64
        ...
        "\n",
        pkts_send_limit_ms, write_kv_trace_length, write_kv_trace_p);
...
static void nc_parse_args_help(void) {
    printf("client [EAL options] --\n"
        "  -p port mask (>0)\n"
        "  -w write ratio ([0, 100])\n"
        "  -z write zipf alpha (e.g., 0, 90, 95, 99)\n"
...
static int nc_parse_args(int argc, char **argv) {
    ...
    printf("parsed arguments: port mask: %"PRIu32
        ", write ratio: %"PRIu32
        ", write zipf alpha: %"PRIu32
        ", kv trace probability: %"PRIu32
        ", pkts send limit ms: %"PRIu64
        "\n",
        enabled_port_mask, write_ratio, write_zipf_alpha,
        write_kv_trace_p, pkts_send_limit_ms);
    return 1;
}
